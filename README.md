...
- **做题页**：大号算式卡片、数字键盘（按范围显示 0~10 或 0~20）、进度条、星星与连对。图片题和故事题会从 8 类主题素材库中生成绘本式小场景、主角、装饰物与任务文案，减少纯算式疲劳。孩子只需点击「帮帮我」，系统会根据范围、题型和答错次数推荐「数一数」「摆一摆」或「小火车走一走」。支持键盘数字 / 退格 / 回车。
...
测试覆盖六种题型各 1000 个样本的合法性、四个范围各 2000 个样本的边界条件，以及学习数据、错题本、机车收藏、难度星星、提示策略、自适应难度和绘本素材等模块。当前共 72 个用例。
