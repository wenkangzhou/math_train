...
- **设置页**：四种练习范围多选（10/20 以内加/减）、六种题型多选（标准题 + 缺项题）、题量（5/10/20）、提示/音效/自动朗读开关。题型会根据所选运算自动启用/置灰；支持高级设置自定义朗读与提示策略。
- **做题页**：大号算式卡片、数字键盘（按范围显示 0~10 或 0~20）、进度条、星星与连对、可视化数量提示（拖拽摆一摆 / 开走动画）。支持键盘数字 / 退格 / 回车。朗读与提示文案由统一的提示策略模块驱动。
...
- **自适应难度**：根据历史正确率与错题分布，动态推荐下一轮的练习范围与题型组合。
- **语音朗读**：基于浏览器 Web Speech API，自动选择中文女声，支持朗读题目与反馈；提示策略模块统一管理题目、缺项、故事场景等不同语境的朗读文本。
...
│   ├── hintStrategy.ts      # 题目→朗读文本/提示文案/拖拽场景
...
测试覆盖六种题型各 1000 个样本的合法性、四个范围各 2000 个样本的边界条件，以及学习数据、错题本、机车收藏、提示策略、自适应难度等模块。当前共 58 个用例。
