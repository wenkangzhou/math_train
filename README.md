...
- **做题页**：大号算式卡片、数字键盘（按范围显示 0~10 或 0~20）、进度条、星星与连对。图片题和故事题会从 8 类主题素材库中生成绘本式小场景、主角、装饰物与任务文案，减少纯算式疲劳。孩子只需点击「帮帮我」，系统会根据范围和题型推荐具体方法；20 以内减法固定使用低识字依赖的十格拿走演示，用两组实物、逐个划走动画、手指操作提示和剩余物品序号表达“拿走”和“还剩”，避免孩子在多种方法与文字说明间切换。支持键盘数字 / 退格 / 回车。
