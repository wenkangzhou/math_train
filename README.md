...
- **奖励系统**：14 辆原创功能机车按累计星星逐步解锁（30 星获得第一辆新伙伴，最终收藏目标 2520 星）；首次答对按难度获得 1～5 星，重试后做对仍获得 1 颗「坚持星」。每辆机车都有与工作匹配的专属行驶场景，例如货运机车经过果园和森林货场、清雪机车穿过雪松林和冰晶桥、港口机车驶过码头与灯塔；完成练习可收集路线邮票并累计到站次数，车库会按最近收益估算距离下一辆机车还需完成多少趟。
...
测试覆盖六种题型各 1000 个样本的合法性、四个范围各 2000 个样本的边界条件，以及学习数据、错题本、机车收藏、专属行驶场景、难度星星、提示策略、20 以内加减法分步策略、自适应难度和绘本素材等模块。当前共 93 个用例。
