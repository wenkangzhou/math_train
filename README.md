...
- **结果页**：庆祝动画、正向评价等级、答对数 / 正确率 / 最长连对 / 提示次数、再练一次 / 马上订正 / 重新选择。
- **奖励系统**：首次答对获得星星，按逐步拉长的里程碑解锁 14 辆不同配色与功能的原创机车；每辆机车都有专属任务路线，完成一轮练习即可到站、收集路线邮票并累计到站次数。
- **长期错题本**：记录题目、曾答答案、累计尝试与最近练习时间；错题按隔天、3天、7天三个阶段复习，三个阶段都独立答对后才归入「已掌握」。
- **答题记录**：保存每轮连续练习的起止时间、用时和题数，并按天展示每日累计及本周累计时间。
