...
- **设置页**：四种练习范围多选（10/20 以内加/减）、六种题型多选（标准题 + 缺项题）、题量（5/10/20）、提示/音效/自动朗读开关。低频设置默认折叠；支持自动难度和手动专项技能。专项技能始终与当前练习范围联动，切换加减法或 10/20 范围时会自动清理不匹配的旧专项，避免设置被隐藏覆盖。
- **做题页**：大号算式卡片、数字键盘（按范围显示 0~10 或 0~20）、进度条、星星与连对。图片题和故事题会从 8 类主题素材库中生成绘本式小场景、主角、装饰物与任务文案，减少纯算式疲劳。孩子只需点击「帮帮我」，系统会根据范围和题型推荐具体方法；20 以内减法固定使用低识字依赖的十格拿走演示，待拿走的实物会先被红色高亮，`↓ −n` 操作按钮紧贴在目标分组下方，拿走后再用序号标明剩余数量，避免抽象图标和多种方法增加理解负担。支持键盘数字 / 退格 / 回车。
- **结果页**：庆祝动画、到站奖励卡片、正向评价等级、首次答对数 / 正确率 / 最长连对 / 提示次数，以及完成奖励和难度加成明细。
- **奖励系统**：14 辆原创功能机车按累计星星逐步解锁（30 星获得第一辆新伙伴，最终收藏目标 2520 星）；首次答对按难度获得 1～5 星，重试后做对仍获得 1 颗「坚持星」。机车不只是涂装不同：快车采用流线车头、货运机车是四轴重型车体、清雪机车带前置雪铲、工程车带检修吊臂、绿电车带受电弓、星河号采用星空舱车体。每辆机车还有与工作匹配的专属行驶场景，例如货运机车经过果园和森林货场、清雪机车穿过雪松林和冰晶桥、港口机车驶过码头与灯塔；完成练习可收集路线邮票并累计到站次数，车库会按最近收益估算距离下一辆机车还需完成多少趟。
...
测试覆盖六种题型各 1000 个样本的合法性、四个范围各 2000 个样本的边界条件，以及学习数据、错题本、机车收藏、专属车体与行驶场景、难度星星、提示策略、专项技能与范围一致性、20 以内加减法分步策略、自适应难度和绘本素材等模块。当前共 98 个用例。
