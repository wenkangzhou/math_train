...
- **做题页**：大号算式卡片、数字键盘（按范围显示 0~10 或 0~20）、进度条、星星与连对。图片题和故事题会从 8 类主题素材库中生成绘本式小场景、主角、装饰物与任务文案，减少纯算式疲劳。孩子只需点击「帮帮我」，系统会根据范围、题型和答错次数推荐具体方法；20 以内加法会用十格图演示“先凑十、再加剩余”，减法会演示“十个一组拿走/破十”，数轴也会按整十分成大步跳，避免逐个拖动或连续点击十几次。支持键盘数字 / 退格 / 回车。
...
测试覆盖六种题型各 1000 个样本的合法性、四个范围各 2000 个样本的边界条件，以及学习数据、错题本、机车收藏、专属车体与行驶场景、难度星星、提示策略、专项技能与范围一致性、20 以内加减法分步策略、自适应难度和绘本素材等模块。当前共 97 个用例。
